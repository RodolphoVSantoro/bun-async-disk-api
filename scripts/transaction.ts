...
export function defaultTransaction(options?: Partial<Transaction>): Transaction {
    return {
...
export function defaultTransactions(options?: Array<Partial<Transaction>>): Transaction[] {
    const transactions: Transaction[] = [];
    for (let i = 0; i < MAX_TRANSACTIONS; i++) {
