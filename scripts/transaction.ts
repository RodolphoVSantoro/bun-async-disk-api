...
import {
  ...
  stringEncode,
  STRING_SIZE,
} from "../src/primitiveEncoders";
import z from "zod";

export const TIPOS_TRANSACTION = ["d", "c"] as const;
export type TipoTransaction = (typeof TIPOS_TRANSACTION)[number];

class TransactionKindError extends Error {}
class InvalidSizeArray extends Error {}

export function isTipoTransactionValid(
  value: unknown
): asserts value is TipoTransaction {
  if (!TIPOS_TRANSACTION.includes(value as TipoTransaction)) {
...
const TipoSchema = z.union([z.literal("c"), z.literal("d")]);
...
export function validateRequest(
  body: unknown
): asserts body is TransactionRequest {
...
function serializeTransaction(transaction: Transaction): Buffer {
  ...
  const serializedDescription = stringEncode(
    transaction.descricao.slice(0, STRING_SIZE).padEnd(STRING_SIZE, "\0")
  );
  const serializedDate = stringEncode(
    transaction.realizada_em.slice(0, STRING_SIZE).padEnd(STRING_SIZE, "\0")
  );
...
function deserializeTransaction(serializedTransaction: Buffer): Transaction {
  ...
  const serializedTipo = serializedTransaction.subarray(
    byteStart,
    byteStart + CHAR_SIZE
  );
  byteStart += CHAR_SIZE;

  const serializedDescription = serializedTransaction.subarray(
    byteStart,
    byteStart + STRING_SIZE
  );
  byteStart += STRING_SIZE;

  const serializedDate = serializedTransaction.subarray(
    byteStart,
    byteStart + STRING_SIZE
  );
...
export function serializeTransactions(transactions: Transaction[]): Buffer {
  if (transactions.length != MAX_TRANSACTIONS) {
    throw new InvalidSizeArray(
      `Transactions has length ${transactions.length}`
    );
  }
...
export function deserializeTransactions(transactionsBuffer: Buffer) {
  ...
  for (let i = 0; i < MAX_TRANSACTIONS; i++) {
    ...
    const transactionBuffer = transactionsBuffer.subarray(
      transactionStart,
      transactionEnd
    );
...
export function defaultTransaction(
  ...
  return {
    ...
    realizada_em: options?.realizada_em ?? "",
  };
}

export function defaultTransactions(
  options?: Array<Partial<Transaction>>
): Transaction[] {
  const transactions: Transaction[] = [];
  for (let i = 0; i < MAX_TRANSACTIONS; i++) {
    transactions.push(defaultTransaction(options?.[i]));
  }
  return transactions;
}
