import { INT_SIZE } from "../src/primitiveEncoders";
import {
  type Transaction,
  defaultTransaction,
...
export function deserializeUser(serializedUser: Buffer): User {
  ...
  const id = deserializeInt(
    serializedUser.subarray(byteStart, byteStart + INT_SIZE)
  );
  byteStart += INT_SIZE;

  const limit = deserializeInt(
    serializedUser.subarray(byteStart, byteStart + INT_SIZE)
  );
  byteStart += INT_SIZE;

  const total = deserializeInt(
    serializedUser.subarray(byteStart, byteStart + INT_SIZE)
  );
  byteStart += INT_SIZE;

  const nTransactions = deserializeInt(
    serializedUser.subarray(byteStart, byteStart + INT_SIZE)
  );
  byteStart += INT_SIZE;

  const oldestTransaction = deserializeInt(
    serializedUser.subarray(byteStart, byteStart + INT_SIZE)
  );
  byteStart += INT_SIZE;

  const transactions = deserializeTransactions(
    serializedUser.subarray(
      byteStart,
      byteStart + TRANSACTION_BYTE_SIZE * MAX_TRANSACTIONS
    )
  );
  ...
  if (transaction.tipo === "d") {
    ...
    if (-1 * newTotal > user.limit) {
      throw new LimitExceedError(
        `The value of ${transaction.valor} exceeds the user limit of ${user.limit}`
      );
    }
    user.total = newTotal;
    return;
